...
module.exports = class UDX extends events.EventEmitter {
  constructor () {
    super()

    this._handle = Buffer.allocUnsafe(binding.sizeof_udx_napi_t)
    this._inited = false
    this._ip = null
    this._port = 0
    this._reqs = []
  ...
  _init () {
    ...
    binding.udx_napi_init(this._handle, this,
      this._onsend,
      this._onmessage,
      this._onclose,
      this._onpreconnect
  ...
  _onpreconnect (id, port, ip) {
    this.emit('preconnect', id, { address: ip, family: 'IPv4', port })
  }
  ...
  bind (port, ip, onlistening) {
    ...
    try {
      this._port = binding.udx_napi_bind(this._handle, port, ip)
      this._ip = ip
    } catch (err) {
      this.emit('error', err)
      return
  ...
  close (onclose) {
    ...
    if (this.bound) binding.udx_napi_close(this._handle)
  ...
  setTTL (ttl) {
    ...
    binding.udx_napi_set_ttl(this._handle, ttl)
  }

  getRecvBufferSize () {
    if (!this._inited) throw new Error('Socket not active')
    return binding.udx_napi_recv_buffer_size(this._handle, 0)
  }

  setRecvBufferSize (size) {
    if (!this._inited) throw new Error('Socket not active')
    return binding.udx_napi_recv_buffer_size(this._handle, size)
  }

  getSendBufferSize () {
    if (!this._inited) throw new Error('Socket not active')
    return binding.udx_napi_send_buffer_size(this._handle, 0)
  }

  setSendBufferSize (size) {
    if (!this._inited) throw new Error('Socket not active')
    return binding.udx_napi_send_buffer_size(this._handle, size)
  }
  ...
  send (buffer, offset, length, port, ip, ttl, onflush) {
    ...
    if (ttl) {
      binding.udx_napi_send_ttl(this._handle, req.handle, id, buffer, port, ip, ttl)
    } else {
      binding.udx_napi_send(this._handle, req.handle, id, buffer, port, ip)
    }
  }

  _allocSend () {
    if (this._free.length > 0) return this._free.pop()
    const handle = Buffer.allocUnsafe(binding.sizeof_udx_send_t)
