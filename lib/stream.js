...
module.exports = class UDXStream extends streamx.Duplex {
  constructor (socket) {
    ...
    this.remoteId = 0
    this.id = binding.udx_napi_stream_init(socket._handle, this._handle, this,
      this._ondata,
      this._onend,
      this._ondrain,
      this._onack,
  ...
  connect (remoteId, port, host) {
    ...
    binding.udx_napi_stream_connect(this._handle, this._readBuffer, remoteId, port, host)
  ...
  _write (buffer, cb) {
    ...
    const drained = binding.udx_napi_stream_write(this._handle, req.handle, id, buffer) !== 0
  ...
  _final (cb) {
    const req = this._allocShutdown()

    const drained = binding.udx_napi_stream_shutdown(this._handle, req.handle) !== 0
  ...
  _predestroy () {
    if (this._handle) binding.udx_napi_stream_close(this._handle)
    this._openContinue(null)
  ...
  _destroy (cb) {
    ...
    if (this.shutdown) binding.udx_napi_stream_close(this._handle)
    this._ondestroy = cb
  }
  ...
  _onshutdown () {
    this.shutdown = true
    this._shreq = null
    if (this.ended) binding.udx_napi_stream_close(this._handle)
  }

  _onclose () {
    // We are not allowed to touch the handle again from now on!
    this._handle = null
    this._destroyContinue(null)
  }
  ...
  _allocShutdown () {
    if (this._shreq !== null) return this._shreq
    const handle = Buffer.allocUnsafe(binding.sizeof_udx_stream_shutdown_t)
    return (this._shreq = { handle })
  }
}
